...
class Sender(object):
  ...
  def run(self):
    ...
    while not self.stop_flag:
      payload = {"node":"", "stat":""}
      for x in range(1,2*(nodes_num+1)):
        if(x == nodes_num+1):
            continue
            
        token = ("0000" + "%X" % (x % (nodes_num + 1)))[-4:]
        payload['node'] = token
        
        payload['stat'] = str(x // (nodes_num + 1))
        
        print(x, (json.dumps(payload).replace(" ","")+"\n").encode('utf-8'))
        self.serial_port.write( (json.dumps(payload).replace(" ","")+"\n").encode('utf-8') )
